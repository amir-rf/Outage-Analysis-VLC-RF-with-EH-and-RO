...
I_b_total = (I_min+I_max)/2 : 0.001:I_max-1e-3;
radius_total = 7;
pr_out_vlc_rf_no = zeros(length(radius_total), length(I_b_total));
for cnt_rad = 1: length(radius_total)
    radious = radius_total(cnt_rad);
    d_u_max = radious;
    d_r_max = radious;
    for cnt_i_b = 1:length(I_b_total)
        ...
        pr_out_vlc_rf_no(cnt_rad, cnt_i_b) = sum(R_VLC_u < R_th)/iteration * (sum(R_VLC_r < R_th)/iteration + sum(R_RF(R_VLC_r >= R_th) < R_th)/iteration);
...
pr_out_vlc = zeros(length(radius_total), length(I_b_total));
pr_out_rf = zeros(length(radius_total), length(I_b_total));
pr_out_vlc_rf = zeros(length(radius_total), length(I_b_total));
E_h_sim = zeros(length(radius_total), length(I_b_total));
for cnt_rad = 1: length(radius_total)
    radious = radius_total(cnt_rad);
    d_u_max = radious;
    d_r_max = radious;
    for cnt_i_b = 1:length(I_b_total)
        ...
        pr_out_vlc(cnt_rad, cnt_i_b) = sum(R_VLC_u < R_th)/iteration;
        pr_out_rf(cnt_rad, cnt_i_b) = sum(R_RF < R_th)/iteration;
        pr_out_vlc_rf(cnt_rad, cnt_i_b) = sum(R_VLC_u < R_th)/iteration * (sum(R_VLC_r < R_th)/iteration + sum(R_RF(R_VLC_r >= R_th) < R_th)/iteration);
        E_h_sim(cnt_rad, cnt_i_b) = mean(E_h);

    end

end

int_val_alpha = zeros(length(radius_total), length(I_b_total));
Pr_out_last_theo_min = zeros(length(radius_total), length(I_b_total));
Pr_out_last_theo_max = zeros(length(radius_total), length(I_b_total));
pr_out_anl = zeros(length(radius_total), length(I_b_total));
E_h_anl = zeros(length(radius_total), length(I_b_total));
int_val = zeros(length(radius_total), length(I_b_total));
int_val_max = zeros(length(radius_total), length(I_b_total));
int_val_min = zeros(length(radius_total), length(I_b_total));
...
for cnt_rad = 1: length(radius_total)
    ...
    for cnt_i_b = 1:length(I_b_total)
        ...
        pr_out_anl(cnt_rad, cnt_i_b) =  1 - ( ((Theta - theta_1)/(theta_2 - theta_1)) .* double(Theta < theta_2) + double(Theta > theta_2))  + ...
        ...
        E_h_anl(cnt_rad, cnt_i_b) = f.*c_h.*integral2(func_vlc_h2_prime ,  0, h_Delta^(-2*(gamma+2)), x_min, x_max)/h_c_prime2;
        f =  eta.*P_LED.*I_b;
        ...
        xf = (2^(R_th/B_RF) - 1)*(N_0*(4*pi*F_c)^2)/(E_h_sim(cnt_rad, cnt_i_b)*c_light^2);
        sigma_h2 = 0.5;
        al_func = @(y) (1 - (  f_1(xf./y) - f_2(xf./y) )    ) .* exp(-y./(2*sigma_h2))  ;
        int_val(cnt_rad, cnt_i_b) =  1/(2*sigma_h2) .* integral(al_func, 0, xf*(2*radius)^beta);
        xf_max = (2^(R_th/B_RF) - 1)*(N_0*(4*pi*F_c)^2)/(E_h_min*c_light^2);
        sigma_h2 = 0.5;
        al_func = @(y) (1 - (  f_1(xf_max./y) - f_2(xf_max./y) )    ) .* exp(-y./(2*sigma_h2))  ;
        int_val_max(cnt_rad, cnt_i_b) =  1/(2*sigma_h2) .* integral(al_func, 0, xf_max*(2*radius)^beta);

        xf_min = (2^(R_th/B_RF) - 1)*(N_0*(4*pi*F_c)^2)/(E_h_max*c_light^2);
        al_func = @(y) (1 - (  f_1(xf_min./y) - f_2(xf_min./y) )    ) .* exp(-y./(2*sigma_h2))  ;
        int_val_min(cnt_rad, cnt_i_b) =  1/(2*sigma_h2) .* integral(al_func, 0, xf_min*(2*radius)^beta);

        xf_alpha = (2^(R_th/B_RF) - 1)*(N_0*(4*pi*F_c)^2)/(E_h_alpha*c_light^2);
        al_func = @(y) (1 - (  f_1(xf_min./y) - f_2(xf_min./y) )    ) .* exp(-y./(2*sigma_h2))  ;
        int_val_alpha(cnt_rad, cnt_i_b) =  1/(2*sigma_h2) .* integral(al_func, 0, xf_alpha*(2*radius)^beta);

        Pr_out_last_theo_min(cnt_rad, cnt_i_b) = pr_out_anl(cnt_rad, cnt_i_b) * (pr_out_anl(cnt_rad, cnt_i_b) + (1-pr_out_anl(cnt_rad, cnt_i_b))*int_val_min(cnt_rad, cnt_i_b));

        Pr_out_last_theo_max(cnt_rad, cnt_i_b) = pr_out_anl(cnt_rad, cnt_i_b) * (pr_out_anl(cnt_rad, cnt_i_b) + (1-pr_out_anl(cnt_rad, cnt_i_b))*int_val_max(cnt_rad, cnt_i_b));

    end
end

figure
plot(I_b_total, pr_out_vlc_rf_no(1,:), '^', 'Color', colors(8,:), 'MarkerIndices',1:20:length(I_b_total))

hold on
grid on
plot(I_b_total, pr_out_vlc_rf(1,:), 'o', 'Color', colors(2,:), 'MarkerIndices',1:20:length(I_b_total))
plot(I_b_total, Pr_out_last_theo_max(1,:), '--', 'Color', colors(1,:))
plot(I_b_total, Pr_out_last_theo_min(1,:), '-.', 'Color', colors(7,:))
plot(I_b_total, pr_out_vlc(1,:), '-', 'Color', colors(4,:))
legend({'$P^{\mathcal{O}}_{\rm H-VR}$ (simulation-fixed orientation)', '$P^{\mathcal{O}}_{\rm H-VR}$ (simulation-random orientation)', '$P^{\mathcal{O}}_{\rm H-VR, UB}$ (analytic-random orientation)', '$P^{\mathcal{O}}_{\rm H-VR, LB}$ (analytic-random orientation)', '$P_{\rm{direct}}^{\mathcal{O}}$ (analytic-random orientation)'},'Interpreter','latex')
xlabel('I_b (A)')
ylabel('Outage probability')
xlim([I_b_total(1) I_b_total(end)])
ax=gca;
ax.GridAlpha=1;
ax.GridLineStyle=':';
ax.MinorGridAlpha=1;
